...
length(x)  # What does length() do? shows number of elements in an 
# object
length(y)
...
ls()
rm(list=ls()) # Danger! What does this do? removes all the objects from
# the workspace
...
matrix(c(1,2,3,4),2,2,byrow=TRUE) # What about byrow=F? fills the matrix by 
# column instead of row

sqrt(x) # What does x look like? it's a 2*2 matrix with 1,2,3,4 filled
# by row
...
# generates 50 random numbers that are part of the normal distribution 
# with mean = 50, SD = 0.1
...
rnorm(50)
set.seed(3) # Try different seeds? gives different sequences of random numbers
y=rnorm(100)
