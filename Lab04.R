...
# png format has a larger image than the pdf format 

# Plot another variable carat

ggplot(data = diam_tb) +
  geom_histogram(mapping = aes(x = carat), binwidth = 0.5) + coord_cartesian(xlim = c(0, 3))
  

# Exercise 2
# Can you improve the visualization of this chart? (changed the 
# x axis limits)
# What is the difference between barchart and histogram?
# histogram shows distribution of a variable, bar charts are used to 
# compare variables
...
ggplot(data = smaller, mapping = aes(x = carat, colour = cut)) +
  geom_freqpoly(binwidth = 0.1) + scale_colour_brewer(palette = "Set1") 

# Exercise 3
# Can you change colors? (edited above)
